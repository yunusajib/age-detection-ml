...
st.markdown(
    "Upload a facial image to predict **age**, **gender**, and **ethnicity**")
...
with st.sidebar:
    ...
    st.markdown(
        "[Model Details](https://github.com/yunusajib/age-detection-ml#readme)")
...
with col2:
    ...
    if uploaded_file is not None:
        ...
        def load_model():
            try:
                ...
                try:
                    ...
                    model = keras.models.load_model(
                        'src/models/best_model_improved.keras',
                        custom_objects={
        ...
        if error:
            ...
            st.warning(
                "⚠️ Note: These are sample predictions for demonstration. Deploy with actual model for real predictions.")
            ...
            with col_c:
                ...
                st.metric(
                    "Ethnicity", f"Class {eth_class}", delta=f"{eth_confidence:.1f}% confidence")

            # Detailed probabilities
            with st.expander("📊 Detailed Probabilities"):
                st.markdown("**Gender Probabilities:**")
                st.progress(float(gender[0]),
                            text=f"Male: {gender[0]*100:.1f}%")
                st.progress(float(gender[1]),
                            text=f"Female: {gender[1]*100:.1f}%")

                st.markdown("**Ethnicity Probabilities:**")
                for i in range(5):
                    st.progress(
                        float(ethnicity[i]), text=f"Class {i}: {ethnicity[i]*100:.1f}%")
